/**
 * Wrapper around the optional tool CLI (`gsk` / `@genspark/cli`) — search / image generation /
...
/** API key written by gsk login (or GSK_API_KEY env var); '' when not logged in. Also used for ArkOffice LLM proxy auth. */
...
export async function gskGenerateImage(
  ...
  // Bare arkoffice file URLs (/api/files/) return 403; swap for a signed direct link with a token
...
/**
 * Swaps a arkoffice file wrapper URL for an anonymously downloadable signed direct link (`gsk download`).
 * Non-arkoffice file URLs are returned as-is; on swap failure the URL is also returned as-is
...
// ── Cloud single-slide generation (ArkOffice slide_generate) ─────────
 ...
 */
const GSK_TOOL_CLI_BASE = 'https://example.com/api/tool_cli'
...
async function toolCliPost(
  ...
  const key = gskApiKey()
  if (!key) throw new Error('Optional cloud CLI is not signed in (gsk login)')

 ...
 * userData dir (via ARKOFFICE_USER_DATA) so runs never touch real settings
...
interface LaunchOptions {
  ...
  userDataDir?: string
  /** UI language override (ARKOFFICE_LANG); defaults to English for stable assertions */
  lang?: string
...
export async function launchShell(options: LaunchOptions): Promise<LaunchedApp> {
  ...
  const app = await electron.launch({
    ...
    env: {
      ...hostEnv,
      ARKOFFICE_USER_DATA: userDataDir,
      ARKOFFICE_LANG: options.lang ?? 'en',

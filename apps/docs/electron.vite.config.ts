...
export default defineConfig({
  ...
  // @arkoffice/* deps ship as raw TS source with extensionless imports, so they
  // must be bundled — externalizing them yields ERR_MODULE_NOT_FOUND under Node
  // (same setup as apps/slides).
  main: {
    plugins: [externalizeDepsPlugin({ exclude: ['@arkoffice/electron-utils'] })],
  },
  preload: {},
  renderer: {
    plugins: [react()],
    server: {
      // Overridable so multiple arkoffice dev instances can coexist (default 5173).

...
type Phase = 'thinking' | 'replying' | 'working' | 'queued'
...
export function AiPanel({
    ...
    loopRef.current = new AgentLoop({
      ...
      events: {
        ...
        onQueue: (info) => {
          ...
          if (pos !== null && pos > 0) setPhase('queued')
        },
  ...
  const typingLabel =
    phase === 'queued' && queuePosition != null && queuePosition > 0
      ? t('aiQueueWaiting', { n: String(queuePosition) })
      : phase === 'replying'
        ? t('aiReplying')
        : phase === 'working'
          ? t('aiWorking')

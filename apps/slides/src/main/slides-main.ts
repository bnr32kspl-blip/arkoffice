/**
 * ArkOffice Slides main process — pptx parsing/render-tree building/edit application/saving all live
...
import { getUiLang, normalizeLang, setUiLang } from '@arkoffice/i18n'
...
import { buildRenderSlide, EMU_PER_PX_96, type RenderSlide } from '@arkoffice/pptx-render'
...
/**
 * Recovery drafts for never-saved decks (wcId → visible path in <Documents>/ArkOffice):
...
/** Directory where AI-generated drafts are saved: <Documents>/ArkOffice/ */
function getDraftsDir(): string {
  return join(app.getPath('documents'), 'ArkOffice')
}
...
/**
 * Auto-save the draft to <Documents>/ArkOffice/<name>.pptx after AI generation completes.
...
export function registerSlidesIpc(): void {
  ...
  ipcMain.handle('slides:copy-slide', (e, slideIndex: number, pngBase64?: string) => {
    ...
    clipboard.writeBuffer('io.arkoffice.slides.slide', Buffer.from('1'))
    return true
  })
  ...
  ipcMain.handle('slides:clipboard-external', () => {
    ...
    if (slideClipboard && marker('io.arkoffice.slides.slide')) return { kind: 'slide' }
    if (marker('io.arkoffice.slides.elements')) return { kind: 'internal' }
  ...
  ipcMain.handle('slides:copy-elements', (e, op: CopyElementsOp) => {
    ...
    if (items.length) {
      ...
      clipboard.writeBuffer('io.arkoffice.slides.elements', Buffer.from('1'))
    }
    return items.length
  })
...
export function startSlidesStandalone(): void {
  ...
  // ARKOFFICE_USER_DATA: test drivers point this at a scratch dir so automated
  ...
  if (!app.isPackaged && process.env.ARKOFFICE_USER_DATA) {
    app.setPath('userData', process.env.ARKOFFICE_USER_DATA)
  }
